...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let fd_path = fs::read_link(format!("/proc/{pid}/fd/{fd}")).ok()?;
        let display_name = OpenFile::path_to_name(fd_path.to_str()?);
        ...
        Some(OpenFile::new(display_name, cursor, access_mode))
    }
